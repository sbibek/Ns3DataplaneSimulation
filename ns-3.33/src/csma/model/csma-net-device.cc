...
CsmaNetDevice::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::CsmaNetDevice")
    ...
    .AddTraceSource ("PhyTxDrop", 
                     ...
                     "ns3::Packet::TracedCallback")
    .AddTraceSource ("General", 
                     "Trace for all the events"
                     "any of the events occurred",
                     MakeTraceSourceAccessor (&CsmaNetDevice::m_generalTrace),
                     "ns3::CSMANetDevices::TracedCallback")
...
uint32_t CsmaNetDevice::getCurrentQueueOccupancy() {
  return m_queue->GetNPackets();
}

uint32_t CsmaNetDevice::getCurrentQueueSize() {
  QueueSize sz = m_queue->GetMaxSize();
  return sz.GetValue();
}

...
PacketSink::PacketSink(Callback<void> onConnectionCloseCB)
{
  NS_LOG_FUNCTION (this);
  m_socket = 0;
  m_totalRx = 0;
  this->onConnectionClosedCallback = &onConnectionCloseCB;
}
...
{
  NS_LOG_FUNCTION (this << socket);
  // if there is a callback available, just call it
  if(this->onConnectionClosedCallback != NULL){
    Callback<void> cb = *onConnectionClosedCallback;
    cb();
  }

...
void BulkSendApplication2::SendHeader (const Address &from, const Address &to)
{
  NS_LOG_FUNCTION (this);
    ...
    NS_LOG_DEBUG("DEBUG NodeId=" << m_node->GetIdx() << " Sending total bytes of data to expect to the receiver which is " << m_maxBytes << "bytes");
...
void BulkSendApplication2::TestSendData (const Address &from, const Address &to)
{
  ...
  if(m_totBytes >= m_maxBytes && !m_transferCompleted) {
      ...
      NS_LOG_DEBUG("RESULT NodeId=" << m_node->GetIdx() << " Time taken to complete the transfer = " << elapsed.GetMilliSeconds() << " ms" );
  }
}
...
void BulkSendApplication2::ReceivedDataCallback(Ptr<Socket> socket) {
  ...
  void BulkSendApplication2::NormalCloseCallback (Ptr<Socket> socket) {
    if(m_responseStarted) {
      ...
      NS_LOG_DEBUG("RESULT NodeId=" << m_node->GetIdx() << " Time taken to receive the data = " << elapsed.GetMilliSeconds() << " ms" );
    }
...
void BulkSendApplication2::SendQuery(void) {
  ...
  NS_LOG_DEBUG("DEBUG NodeId=" << m_node->GetIdx() << " Sent node ranking query to the schedular \n");
...
void BulkSendApplication2::QueryResponseHandler(Ptr<Socket> socket) {
  ...
  while ((packet = socket->RecvFrom (from)))
    {
      ...
      if (packet->GetSize () > 0) {
          ...
          NS_LOG_DEBUG("DEBUG NodeId=" << m_node->GetIdx() << " " << response.GetCount() << " " << "responses received from schedular\n");
          // NS_LOG_DEBUG("[Rx Query Respone (sender)] count=" << response.GetCount());
          std:: cout << Simulator::Now() <<  " Response received " << response.GetCount() << std::endl << std::flush;
          QueryValue value;
          for(int i=0;i<response.GetCount();i++) {
            packet->RemoveHeader(value);
            NS_LOG_DEBUG("DEBUG NodeId=" << m_node->GetIdx() << " { nodeid=" << value.GetNodeId() << " value=" << value.GetValue() << "}");
  ...
  if(results.size() > 0) {
    ...
    Simulator::Schedule(Seconds(0.0), &BulkSendApplication2::DataTransferSequence, this, std::get<0>(results.at(0)));
  }

}

void BulkSendApplication2::QuerySequence(void) {
  Simulator::Schedule(Seconds(0.0), &BulkSendApplication2::SendQuery, this); 
}

void BulkSendApplication2::DataTransferSequence(int nodeId) {
  ...
  //  m_peer = ns3::Address(ip->GetAddress(nodeId));
   ...
   NS_LOG_DEBUG("DEBUG NodeId=" << m_node->GetIdx() << " Initiating transfer to Node(" << nodeId << ") IP="<<peerAddressStringStream.str());
   Simulator::Schedule(Seconds(0.0), &BulkSendApplication2::InitTransfer, this);
}


void BulkSendApplication2::InitTransfer(void) {
  if (!m_socket)
    {

...
#include "ns3/gtcp-server.h"
#include "ns3/gtcp-client.h"
namespace ns3 {
/**
 * \ingroup GtcpClientserver
 ...
 */
class GtcpServerHelper
{
public:
  /**
   * Create GtcpServerHelper which will make life easier for people trying
   ...
   */
  GtcpServerHelper (uint16_t port);
   ...
   */
  Ptr<GtcpServer> GetServer (void);
private:
  ObjectFactory m_factory; //!< Object factory.
  Ptr<GtcpServer> m_server; //!< The last created server application
};
 ...
 *
 */
class GtcpClientHelper
{

public:
  /**
   * Create GtcpClientHelper which will make life easier for people trying
  ...
  GtcpClientHelper (Address ip, uint16_t port);
  /**
   *  Create GtcpClientHelper which will make life easier for people trying
   * to set up simulations with udp-client-server. Use this variant with
...
*/
class GtcpTraceHelper
{
public:
  /**
   * Create GtcpTraceHelper which will make life easier for people trying
   * to set up simulations with udp-client-server.
   *
   */
  GtcpTraceHelper ();
   ...
   */
  GtcpTraceHelper (Address ip, uint16_t port, std::string filename);
  /**
   * Create GtcpTraceHelper which will make life easier for people trying
   * to set up simulations with udp-client-server. Use this variant with
   * addresses that do include a port value (e.g., InetSocketAddress and
   ...
   * \param filename the file from which packet traces will be loaded
   */
  GtcpTraceHelper (Address addr, std::string filename);

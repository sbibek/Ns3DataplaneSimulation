 ...
 */
class ProbeListenerHelper
{
public:
  /**
   * Create ProbeListenerHelper which will make life easier for people trying
   ...
   */
  ProbeListenerHelper (uint16_t port);
   ...
   */
  Ptr<ProbeListener> GetServer (void);
private:
  ObjectFactory m_factory; //!< Object factory.
  Ptr<ProbeListener> m_server; //!< The last created server application
};
 ...
 *
 */
class ProbeSenderHelper
{

public:
  /**
   * Create ProbeSenderHelper which will make life easier for people trying
   ...
   */
 ProbeSenderHelper (Address ip, uint16_t port);
  ProbeSenderHelper (Address ip, uint16_t port, uint16_t max_p, float interval);
  /**
   *  Create ProbeSenderHelper which will make life easier for people trying
   * to set up simulations with udp-client-server. Use this variant with
  ...
  ProbeSenderHelper (Address addr);
...
*/
class ProbeTraceClientHelper
{
public:
  /**
   * Create ProbeTraceClientHelper which will make life easier for people trying
   * to set up simulations with udp-client-server.
   *
   */
  ProbeTraceClientHelper ();
   ...
   */
  ProbeTraceClientHelper (Address ip, uint16_t port, std::string filename);
  /**
   * Create ProbeTraceClientHelper which will make life easier for people trying
   * to set up simulations with udp-client-server. Use this variant with
   * addresses that do include a port value (e.g., InetSocketAddress and
   ...
   * \param filename the file from which packet traces will be loaded
   */
  ProbeTraceClientHelper (Address addr, std::string filename);

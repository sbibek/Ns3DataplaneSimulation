...
public:
  ...
  SwitchNetDevice ();
  virtual ~SwitchNetDevice ();

  // ADDED BY BIBEK
  __stats getCurrentStats(Ptr<NetDevice> incomingPort);

  /** 
   * \brief Add a 'port' to a bridge device
   * \param bridgePort the NetDevice to add
   *
   * This method adds a new bridge port to a SwitchNetDevice, so that
   ...
   */
  void AddBridgePort (Ptr<NetDevice> bridgePort, uint64_t portId);
...
private:
   ...
   */
  SwitchNetDevice (const SwitchNetDevice &);
   ...
   * \returns
   */
  SwitchNetDevice &operator = (const SwitchNetDevice &);
